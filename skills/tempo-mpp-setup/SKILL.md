...
Buka terminal WSL Ubuntu biasa, pastikan CLI Tempo sudah terinstall dan kunci API/mnemonic sudah tersimpan di `~/.tempo/env` di dalam WSL kamu.

### 2. Verifikasi Koneksi WSL dari Hermes
...
### 3. Menjalankan Pembayaran MPP via Hermes

Saat kamu ingin Hermes memanggil DeepSeek atau Exa menggunakan Tempo, kamu tidak perlu menginstall node_modules di Windows. Kamu cukup menyuruh Hermes untuk menjalankan script atau perintah CLI di dalam WSL.

Karena Hermes terkadang sulit mengingat command panjang, buatlah folder proyek sederhana di dalam WSL (atau map folder Windows kamu ke `/mnt/c/Users/cruzl/...`).

Contoh tata cara menjalankan query DeepSeek via MPP dari Hermes di WSL:

```bash
wsl -d Ubuntu -- bash -lc 'cd /mnt/c/Users/cruzl/tempo-telegram-bot && source ~/.tempo/env && tempo mpp execute --agent deepseek --prompt "Cek harga ETH hari ini"'
```

### 4. Membuat Shortcut / Skill yang Memicu WSL

Di folder proyekmu (misalnya di `C:/Users/cruzl/tempo-telegram-bot`), kamu bisa menambahkan file bash sederhana bernama `run_mpp.sh`:

```bash
#!/bin/bash
source ~/.tempo/env
tempo mpp execute --agent $1 --prompt "$2"
```

Kemudian di dalam Hermes, kamu bisa menggunakan command ini langsung:

```bash
wsl -d Ubuntu -- bash -lc 'cd /mnt/c/Users/cruzl/tempo-telegram-bot && bash run_mpp.sh deepseek "Kenapa crypto crash?"'
```
